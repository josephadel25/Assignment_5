...
def _read_classes_file():
    """
    Read target classes for TEST from a text file if provided.
    Lookup order:
      1) Environment variable LASOT_TEST_CLASSES_FILE
      2) Environment variable LASOT_CLASSES_FILE (fallback)
      3) classes_test.txt in current working directory
      4) classes.txt in current working directory
    File format: one class name per line; empty lines and lines starting with '#' are ignored.
    """
    candidates = []
    env_test = os.environ.get('LASOT_TEST_CLASSES_FILE', '').strip()
    env_common = os.environ.get('LASOT_CLASSES_FILE', '').strip()
    if env_test:
        candidates.append(env_test)
    if env_common:
        candidates.append(env_common)
    candidates.append(os.path.join(os.getcwd(), 'selected_classes_test.txt'))
    candidates.append(os.path.join(os.getcwd(), 'selected_classes.txt'))
...
class LaSOTDataset(BaseDataset):
    ...
    def __init__(self):
        ...
        # Optional: filter to classes provided in a txt file
        target_classes = _read_classes_file()
        if target_classes is not None:
            orig = len(self.sequence_list)
            self.sequence_list = [s for s in self.sequence_list if s.split('-')[0] in target_classes]
            print(f"[TEST] LaSOT class filter: {orig} -> {len(self.sequence_list)} sequences "
